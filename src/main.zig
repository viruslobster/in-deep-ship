...
fn play(
    ...
    try col_writer.print("Player: foo\n\n\n", .{});
    try col_writer.print("{s}\n", .{grid_template});
    try col_writer.flush();

    var spacer = try Layout.Column.init(gpa, 1, 100);
    const spacer_writer = &spacer.interface;
    try spacer_writer.splatByteAll(' ', 5);

    const layout = Layout.init(&.{ &spacer, &column, &spacer, &column });
    try stdout.print("{f}", .{layout});
    //try Layout.write(&.{ &spacer, &col0, &col0 }, stdout);
    ...
    for (0..board.placements.len) |i| {
        ...
        const x = 5 + 4 + placement.x * 6;
        const y = 3 + 3 + placement.y * 3;
